...
DASHBOARD_UID=SaltNodeDashboard
DASHBOARD_FILE="./prometheus/salt-node-exporter-full.json"
...
for region in $(cat "${TARGET_REGIONS_FILE}"); do
    ...
    echo "Deleting previous Salt node dashboard"
    echo "-----------------------------------"
    ...
    echo ""
    echo "Creating new Salt node dashboard"
    echo "-----------------------------"
...
echo "Complete deployment of the Salt node dashboard to all ${ENV} regions"
