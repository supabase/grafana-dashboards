...
DASHBOARD_UID=HTTPRequests
DASHBOARD_FILE="./prometheus/http-requests.json"
...
for region in $(cat "${TARGET_REGIONS_FILE}"); do
    ...
    echo "Deleting previous HTTP Requests dashboard"
    echo "-----------------------------------"
    ...
    echo ""
    echo "Creating new HTTP Requests dashboard"
    echo "-----------------------------"
...
echo "Complete deployment of the HTTP Requests dashboard to all ${ENV} regions"
